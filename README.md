...
>   1.代码，名字  
>   2.标签 - 显示标签，以逗号分隔，双击打开标签编辑窗口  
>   3.笔记 - 显示最后一条笔记，双击打开笔记编辑窗口  
>   4.K线图表 - 双击打开K线图表，并在上面按时间列出笔记  
>   5.盯盘相关（TBD）  
>   6.分析 - 显示此证券的分析结果  
>   7.工具 - 双击打开工具面板 
>   
...
>   
> 数据存储：  
>   1.默认保存到“系统用户目录/StockAnalysisSystem”下（上下文中简称为“用户目录”）  
>   2.用户可选择保存路径，文件转移由用户自己处理，用户选择的目录会保存到config.json中  
>   3.所有StockMemo保存到同一个csv文件  
